...
docker run --rm -it \
    --entrypoint /bin/sh \
    -e HOST_UID=`id -u` \
    -v /home/pch330/share/naver_project/prompt_ood:/root \
    zxcvvxcz/nv-prompt:base \
    -c 'chown -R ${HOST_UID}:${HOST_UID} /root'

docker build -t nv-prompt:gpt2-large -f Dockerfile .

# make container
docker stop test_container1
docker rm test_container1
docker run -itd --name test_container1 -p 8887:8887 --ipc=host --gpus all --restart=always nv-prompt:gpt2-large
